...
<div class="container">
		<form action="SaveMagicianSettingsExchange" method="POST" id="accountForm" enctype="multipart/form-data">
		<H1 style="margin-left: 15%;">Upload New Image For Main Page</H1>
		<hr style="height:1px;border:none;color:#d3d3d3;background-color:#d3d3d3;">
		<fieldset>
		<input type="hidden" name="enter" value="Save New Profile Image" style="display: none;">
			<div class="form-group row"> 	
				<label class="col-md-8 control-label" for="file">Select below to upload a new image: </label>
			</div>
			<div class="form-group row"> 	
				
					<input type="file" name="file" id="file" size="50000" style="margin-left: 3%">
			...
			<div class='btn-group pull-left' role='group' aria-label='...' style='margin-top:10px; margin-bottom: 10px;'>
			  	<a href="cart.jsp" type='button' class='btn btn-default'><span class='fa fa-times'></span>&nbsp;Cancel</a>
			  	<button type='submit' class='btn btn-default btn-success' id="saveImage"><i class="fa fa-upload" aria-hidden="true"></i>&nbsp;Upload Image</button>
			  	</div>
			</div> 
			</fidldset>
		</form>  
...
$("#accountForm").submit(function(event){
    ...
    if (!file) {
    	...
    	bootbox.confirm({
    	    ...
    	    message: "Changing image will overwrite your previous image. Would you like to continue?",
    	    buttons: {
    	        cancel: {
